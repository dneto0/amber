...
TEST_F(DawnComputePipelineInfoTest, DefaultValueHasNoShader) {
  ...
  EXPECT_FALSE(bool(cpi.compute_shader));
}
...
TEST_F(DawnRenderPipelineInfoTest, DefaultValuesForMembers) {
  ...
  EXPECT_FALSE(bool(rpi.vertex_shader));
  EXPECT_FALSE(bool(rpi.fragment_shader));
  ...
  EXPECT_FALSE(bool(rpi.fb_texture));
  EXPECT_FALSE(bool(rpi.fb_buffer));
